...
parser.add_argument('--superuser', help ="Name of user on server with admin privlages.")
## NEED TO BE ABLE TO SSH IN AS THIS USER AND ESCALATE
parser.add_argument('-u','--usr', metavar='raccoon', help='Name of firo daemon user external server (default: firo).',type=str)
parser.add_argument('-p','-passwd',help="Password of non-privladged firo daemon user.")
### Normal user shouldn't be an option, this requires root force it at start.
...
parser.add_argument('--no-daemon-user', action='store_true', help='Use super user as the firo-daemon user.')
